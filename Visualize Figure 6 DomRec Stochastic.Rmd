...
```{r, echo=FALSE}
#setwd("C:/Users/beallma/Documents/MATLAB/Chicago_data")
#test.summ.data = summarized.data.frame(N = 1000, C = '10000', k = '1000000',MOI = 0.1, heterogeneity = 1)
#PANEL A: TIME COURSE
n = 1000; moi = .1; moi2 = 5.9948
#k.titles = c(1, 19, 1000, 51795, '1000000')#
k.titles = c(1, 32, 1000, 31623, '1000000')#c(1, 3, 7, 19, 52, 139, 373, 1000, 2683, 7197, 19307, 51795, 138950, 372759, '1000000')
k.values = c(0.01, 0.3612, 10, 316.23, 10000)

Fig2.A.data = data.frame()

for (K in k.titles) {
  print(K)
  # read in the mean data for each time point and for each replicate
  data.curr.k = summarized.data.frame(N = n, MOI = moi, k = K, heterogeneity = 1, t.end = 150)
  Fig2.A.data = rbind.data.frame(Fig2.A.data, data.curr.k)
  
}

#setwd("C:/Users/beallma/Documents/MATLAB/coinfection_heterogeneity")
null.no.het = summarized.data.frame(N = n, MOI = moi, heterogeneity = 0, reps = 20, t.end = 150)
null.no.het$k = Inf
null.no.het = subset(null.no.het, t < 21)
Fig2.A.data = subset(Fig2.A.data, t < 21)
Fig2.A.data.combo = rbind.data.frame(Fig2.A.data, null.no.het)
#max(Fig2.A.data$t))
#calculate the mean, sd, and 95% CI for each of the MOIs across replicates
#Fig2.A.data.ends.SE = summarySE(Fig2.A.data.combo, measurevar="num_mut", groupvars=c("t", "k"))
#Fig2.A.data.ends.SE$k = unique(Fig2.A.data.combo$k)
legend_labs = c(k.values, Inf)#c(log10(min(subset(Fig2.A.data.ends.SE$k, is.na(Fig2.A.data.ends.SE$k) == FALSE))),"","","","","","",
              #  log10(median(unique(subset(Fig2.A.data.ends.SE$k, is.na(Fig2.A.data.ends.SE$k) == FALSE)))),
              #  "","","","","","",log10(max(subset(Fig2.A.data.ends.SE$k, is.na(Fig2.A.data.ends.SE$k) == FALSE))))
#legend_labs = log10(as.integer(k.titles))

pd <- position_dodge(0)
alph = 0.4

Fig2.A.plot = ggplot(NULL, aes(x=t, y=mean_mut)) + 
    geom_line(data = Fig2.A.data.combo, aes(x=t, y=mean_mut,  color = as.factor((k)), group = as.factor((k))), position=pd, alpha = alph) + 
    geom_point(data = Fig2.A.data.combo, aes(x=t, y=mean_mut, color = as.factor((k)), group = as.factor((k))), position=pd, size=3, alpha = alph, show.legend = TRUE)+
    geom_errorbar(data = Fig2.A.data.combo, aes(x=t, ymin=mean_mut-std_error, ymax=mean_mut+std_error, color = as.factor((k)), group = as.factor(k)), width=1, position = pd, alpha = alph) +
  xlab("t") + ylab("mean mutations \nper virion") +  
  #scale_fill_discrete(name = "",labels = "No \nheterogeneity")+
  scale_color_manual(name = "k",values = c(cbPalette[1:length(k.values)], "black"),#c(brewer.pal(n = 5, name = "Dark2"), "black"), 
                     labels = c(0.01, 0.3612, 10, 316.23, 10000, "No \nHeterogeneity"), 
                     breaks = as.factor(unique((Fig2.A.data.combo$k)))) +
  theme_bw() +
...
for (n in N.2.B) {
  setwd("C:/Users/beallma/OneDrive - Emory University/Documents/MATLAB/GitHubCode/max")
  null.no.het = rbind.data.frame(null.no.het, summarized.data.frame(N = n, MOI = moi, heterogeneity = 0, reps = 20, t.end = 20))
  
  setwd("C:/Users/beallma/OneDrive - Emory University/Documents/MATLAB/GitHubCode/max211007")
  for (K in k.titles) {
    # read in the mean data for each time point and for each replicate
    data.curr.k = summarized.data.frame(N = n, MOI = moi, k = K, heterogeneity = 1, t.end = 20, reps=20)
...
null.no.het$k = Inf
null.no.het = subset(null.no.het, t == 20)
Fig2.B.data = subset(Fig2.B.data, t == 20)
...
null.no.het = data.frame()
#setwd("C:/Users/beallma/OneDrive - Emory University/Documents/MATLAB/GitHubCode/base")
for (cell in C) {
    setwd("C:/Users/beallma/OneDrive - Emory University/Documents/MATLAB/GitHubCode/base211007")
    null.no.het = rbind.data.frame(null.no.het, summarized.data.frame(N = n, C = cell, heterogeneity = 0, reps = 20, t.end = 20))
    setwd("C:/Users/beallma/OneDrive - Emory University/Documents/MATLAB/GitHubCode/max")
    for (K in k.titles) {
    #print(K)
    # read in the mean data for each time point and for each replicate
      #data.curr.k = make.data.frame(N = n, C = cell, k = K, heterogeneity = 1, t.end = 20, reps = 20)
    data.curr.k = summarized.data.frame(N = n, C = cell, k = K, heterogeneity = 1, t.end = 20, reps = 20)
...
null.no.het$k = Inf
null.no.het = subset(null.no.het, t == 20)
Fig2.C.data = subset(Fig2.C.data, t == 20)
# extract only the final time points from the data
...
for (N in n) {
  print(N)
  setwd("C:/Users/beallma/OneDrive - Emory University/Documents/MATLAB/GitHubCode/max211007")
  null.no.het = rbind.data.frame(null.no.het, summarized.data.frame(N = N, C = c, heterogeneity = 0, reps = 20, t.end = 20))
  setwd("C:/Users/beallma/OneDrive - Emory University/Documents/MATLAB/GitHubCode/max")
  for (K in k.titles) {
    print(K)
    ...
    # }
    data.curr.k = summarized.data.frame(N = N, C = c, k = K, heterogeneity = 1, t.end = 21)
...
null.no.het$k = Inf
null.no.het = subset(null.no.het, t == 20)
Fig2.D.data = subset(Fig2.D.data, t == 20)
# extract only the final time points from the data
...
F2.paper
#F2.supplement
ggsave(plot = F2.paper, file = paste0("Figure_Rec_Stochastic_", Sys.Date(), ".jpg"), height = 3.5, width = 6, units = "in")
ggsave(plot = F2.paper, file = paste0("Figure_Rec_Stochastic_", Sys.Date(), "pdf"), height = 3.5, width = 6, units = "in")
#ggsave(plot = F2.supplement, file = paste0("Figure 2 Supplement", Sys.Date(), ".jpg"), height = 4.4, width = 6, units = "in")
...
for (N in n) {
  ...
  for (K in k.titles) {
    ...
    #   n = format(round(c * moi), scientific = FALSE)
    # }
    data.curr.k = summarized.data.frame(N = N, C = c, k = K, heterogeneity = 1, t.end = 20)
    Fig2.D.data = rbind.data.frame(Fig2.D.data, data.curr.k)
  }
}
...
F2.paper
#F2.supplement
ggsave(plot = F2.paper, file = paste0("Figure_Dom_Stochastic_", Sys.Date(), ".jpg"), height = 3.5, width = 6, units = "in")
ggsave(plot = F2.paper, file = paste0("Figure_Dom_Stochastic_", Sys.Date(), ".pdf"), height = 3.5, width = 6, units = "in")
#ggsave(plot = F2.supplement, file = paste0("Figure 2 Supplement", Sys.Date(), ".jpg"), height = 4.4, width = 6, units = "in")
